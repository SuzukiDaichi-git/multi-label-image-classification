...
step_size = 40
num_epoch = 40
result_dir = 'results_possoft_neghard_gt'
resume = None # './results/40_40_img224_batch128_lr0.1_step40_nepoch40_begin40_cosann_epoch10_checkpoint.pth.tar'
update_begin = 15
num_pred = 10
num_classes=20

exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_begin{7}_cosann_possoftneghard".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, update_begin)
    ...
    def label_update(self, results=None, epoch=None):
        ...
        if epoch+1 >= self.begin:
            ...
            for c in range(num_classes):
                ...
                indexes_pos_high = idx_argsorted[max_idx_pos[c]:, c]
                indexes_neg_low = idx_argsorted[:max_idx_neg[c], c]
                indexes_neg_high = idx_argsorted[max_idx_neg[c]:, c]
                ...
                self.soft_labels[indexes_neg_low, c] += 0 * (1-self.labels[indexes_neg_low, c])
