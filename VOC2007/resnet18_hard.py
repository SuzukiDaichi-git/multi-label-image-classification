...
exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_begin{7}_prate{8}_nrate{9}_cosann".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, update_begin, pos_rate, neg_rate)
...
class VocDataset(Dataset):
    ...
    def label_update(self, results=None, epoch=None):
        ...
        if epoch+1 >= self.begin:
            ...
            results_sorted = np.sort(results, axis=0)
            ...
            thresh_pos = np.zeros(num_classes)
            thresh_neg = np.zeros(num_classes)
            
            self.soft_labels[:,:] = 0
            for c in range(num_classes):
                
                idx_pos = np.where(labels_sorted[:,c]==1)[0]
                idx_neg = np.where(labels_sorted[:,c]==0)[0]
                
                thresh_pos[c] = results_sorted[idx_pos[int(len(idx_pos)*pos_rate)],c]
                thresh_neg[c] = results_sorted[idx_neg[int(len(idx_neg)*neg_rate)],c]
                
                indexes_pos_high = np.where(results[:,c]>thresh_pos[c])
                indexes_neg_high = np.where(results[:,c]>thresh_neg[c])
            ...
            with open(save_thresh_pos_path, 'a') as f:
                w = csv.writer(f)
                w.writerow([epoch+1]+list(thresh_pos))
            with open(save_thresh_neg_path, 'a') as f:
                w = csv.writer(f)
                w.writerow([epoch+1]+list(thresh_neg))
