...
# exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_cbegin{7}_ubegin{8}_cosann_clean_update".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, clean_begin, update_begin)
exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_cbegin{7}_cosann_clean_gt".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, clean_begin)
...
class VocDataset(Dataset):
    ...
    def label_update(self, results=None, epoch=None):
        # thresh_pos, thresh_neg, soft_labelsを更新
        ...
        for c in range(num_classes):
            self.thresh_pos[c] =  results[idx_argsorted[max_idx_pos[c],c],c]
            self.thresh_neg[c] =  results[idx_argsorted[max_idx_neg[c],c],c]
