...
exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_begin{7}_alpha{8}_beta{9}_cosann_jo".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, update_begin, alpha, beta)
...
class my_criterion():
    def __init__(self, alpha=0.0, beta=0.0):
        ...
        annotation = pd.read_csv(os.path.join('annotation','{0}_{1}_trainAnnotation.csv'.format(missing, overlabeling)), header=None).iloc[1:,1:].values.astype('double')
...
criterion = my_criterion(alpha, beta)
