...
step_size = 25
num_epoch = 50
result_dir = 'results_reg_resnet18_kai'
...
exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_alpha{7}_beta{8}_jo_kai".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, alpha, beta)
...
class my_criterion():
    ...
    def __call__(self, preds, targets):
        ...
        prob_entropy = -torch.mean(torch.mean(((1-probs) * torch.log(1-probs)), 1), 0)
...
class my_criterion2():
    ...
    def __call__(self, preds, targets):
        bce = self.bce(preds, targets)
        probs = torch.sigmoid(preds)
        prob_entropy = -torch.sum(probs*torch.log(probs)*targets)/torch.sum(targets) - torch.sum((1-probs)*torch.log(1-probs)*(1-targets)) / torch.sum(1-targets)
        prob_avg = torch.mean(probs, dim=0)
        prob_dist = -torch.mean(self.anno_dist * torch.log(prob_avg))
