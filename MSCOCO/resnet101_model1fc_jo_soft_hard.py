...
pos_thresh = 0.6
neg_thresh = 0.3
result_dir = 'results_proposal'
resume = 'results2/20_20_img224_batch16_lr0.1_step20_nepoch40_epoch10_checkpoint.pth.tar'
# summary
exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_pos{7}_neg{8}".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, pos_thresh, neg_thresh)
...
class CocoDataset(Dataset):
    ...
    def label_update(self, results, epoch):
        ...
        if epoch+1 >= self.begin: 
            ...
            neg_low_results = mean_labels * (np.array(self.labels)==0) * (mean_labels<neg_thresh)
            pos_low_results = mean_labels * (np.array(self.labels)==1) * (mean_labels<pos_thresh)
            neg_high_results = 1 * (np.array(self.labels)==0) * (mean_labels>=neg_thresh)
            pos_high_results = 1 * (np.array(self.labels)==1) * (mean_labels>=pos_thresh)
            self.soft_labels = neg_low_results + pos_low_results + neg_high_results + pos_high_results
    
    def __getitem__(self,idx):
        img_name = os.path.join(self.image_dir, self.names[idx])
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=40, resume=False):
    ...
    if resume:
        if os.path.isfile(resume):
            ...
            neg_low_results = mean_labels * (np.array(dataloaders['train'].dataset.labels)==0) * (mean_labels<neg_thresh)
            pos_low_results = mean_labels * (np.array(dataloaders['train'].dataset.labels)==1) * (mean_labels<pos_thresh)
            neg_high_results = 1 * (np.array(dataloaders['train'].dataset.labels)==0) * (mean_labels>=neg_thresh)
            pos_high_results = 1 * (np.array(dataloaders['train'].dataset.labels)==1) * (mean_labels>=pos_thresh)
            dataloaders['train'].dataset.soft_labels = neg_low_results + pos_low_results + neg_high_results + pos_high_results
...
torch.save(model_ft.state_dict(),os.path.join(result_dir, exp_cond+'_best{7}_checkpoint.pth.tar'.format(best_cmap)))
