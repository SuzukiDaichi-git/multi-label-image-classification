...
# summary
exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_soft_hard_per_class".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch)
...
class CocoDataset(Dataset):
    ...
    def label_update(self, results, epoch):
        ...
        if epoch+1 >= self.begin: 
            ...
            is_hard= np.zeros(len(self.labels),80)
            for i in range(80):
                for j in range(num_instance[i]):
                    is_hard[results_argsort[j][i]][i] = 1
                    
            self.soft_labels = mean_labels*(1-is_hard) + 1*is_hard
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=40, resume=False):
    ...
    if resume:
        if os.path.isfile(resume):
            ...
            results_argsort = mean_labels.argsort(axis=0)
            is_hard= np.zeros((len(dataloaders['train'].dataset.labels),80))
            for i in range(80):
                for j in range(num_instance[i]):
                    is_hard[results_argsort[j][i]][i] = 1
            dataloaders['train'].dataset.soft_labels = mean_labels*(1-is_hard) + 1*is_hard
