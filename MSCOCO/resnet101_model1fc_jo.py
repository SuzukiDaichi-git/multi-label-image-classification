...
resume = 'results2/0_0_img224_batch16_lr0.1_step20_nepoch40_epoch10_checkpoint.pth.tar'
update_begin = 10
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=40, resume=False):
    ...
    if resume:
        if os.path.isfile(resume):
            ...
            print(dataloaders['train'].dataset.prediction.shape)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        torch.save({'epoch': epoch+1,
                   ...
                   ckpt_path)
        if epoch+1 in [5,8,10,15,20,25,30,35]:
            torch.save({'epoch': epoch+1,
                        'state_dict': model.state_dict(),
                        'optimizer': optimizer.state_dict(),
                        'scheduler': exp_lr_scheduler.state_dict(),
