...
def main(args):
    ...
    # change!
    base_name = '{0}_{1}_img224_batch16_lr0.1_step20_nepoch20_jo_begin10_epoch11'.format(args.missing, args.overlabeling)
...
#     # train/val
    for phase in ['train']:
        ...
        fig.savefig(os.path.join(args.result_dir, base_name+'_{}_probs.jpg'.format(phase)))
...
#         new_img_path = os.path.join(args.sample_dir, base_name, 'train_img', image_datasets['train'].labels.iloc[idx,0])
...
#             writer = csv.writer(f)
#             if i == 0:
#                 writer.writerow(['file_name']+[mscoco_class[str(j)] for j in range(80)])
#             writer.writerow([image_datasets['train'].labels.iloc[idx,0]]+[round(prob[j].cpu().detach().item(), 3) for j in range(80)])
...
#         new_img_path = os.path.join(args.sample_dir, base_name, 'test_img', image_datasets_test.labels.iloc[idx,0])
#         shutil.copy(img_path, new_img_path)
#         model.eval()
...
#             if i == 0:
#                 writer.writerow(['file_name']+[mscoco_class[str(j)] for j in range(80)])
#             writer.writerow([image_datasets_test.labels.iloc[idx,0]]+[round(prob[j].cpu().detach().item(), 3) for j in range(80)])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--missing", default=20, type=int)
    parser.add_argument("--overlabeling", default=20, type=int)
    parser.add_argument("--ckpt_dir", type=str, default="results_hard_soft_per_class_gt")
    parser.add_argument("--result_dir", type=str, default="visualize_resnet18_gt")
    parser.add_argument("--sample_dir", type=str, default="sample_resnet18")
