...
step_size = 20
num_epoch = 40
result_dir = 'results_ASL'
resume = False
update_begin = num_epoch
...
def train_model(model, train_criterion, test_criterion, optimizer, scheduler, num_epochs=40, resume=False):
    ...
    if resume:
        if os.path.isfile(resume):
            ...
            dataloaders['train'].dataset.soft_labels = dataloaders['train'].dataset.prediction.mean(axis=1)
            # exp_lr_scheduler.load_state_dict(checkpoint['scheduler'])
            loss = checkpoint['loss']
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.resume, checkpoint['epoch']+1 if 'epoch' in checkpoint else 'unknown'))
        else:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        test_loss,cmap,emap,p,r,f1 = test_model(model,optimizer,test_criterion,trial)

        # save model
        exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epochs)
...
train_criterion = AsymmetricLossOptimized()
test_criterion = nn.MultiLabelSoftMarginLoss()
...
model_ft, best_cmap = train_model(model_ft, train_criterion, test_criterion, optimizer_ft, exp_lr_scheduler,num_epochs=num_epoch, resume=resume)
