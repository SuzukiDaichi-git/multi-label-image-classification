...
class my_criterion():
    def __init__(self, init_bias=0, neg_scale=2.0):
        self.init_bias = init_bias
        self.neg_scale = neg_scale
        
    def __call__(self, cls_score, label):
        logits = label * (cls_score - self.init_bias) + (1 - label) * (cls_score - self.init_bias) * self.neg_scale
        weight = label + (1 - label) / self.neg_scale
        loss = bce_with_logits(logits, label.float(), weight)
