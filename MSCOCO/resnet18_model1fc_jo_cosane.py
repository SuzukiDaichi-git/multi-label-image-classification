...
step_size = 20
num_epoch = 20
result_dir = 'results_cos_annealing'
...
# exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_alpha{7}_resample_crop".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, alpha, regmax)
# exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_begin{7}".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, update_begin)
# exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_crop{7}".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, crop_range)
...
class my_criterion():
    ...
    def __call__(self, preds, targets):
        ...
        prob_entropy = -torch.mean(torch.mean(max(probs * torch.log(probs), -1./math.e*regmax), 1), 0)
...
class my_criterion2():
    ...
    def __call__(self, preds, targets):
        bce = self.bce(preds, targets)
        probs = torch.sigmoid(preds)
        prob_entropy = -torch.mean(torch.mean(probs**2 * (1-probs), 1), 0)
        prob_avg = torch.mean(probs, dim=0)
        prob_dist = -torch.mean(self.anno_dist * torch.log(prob_avg))
...
# criterion = my_criterion2(alpha=alpha, beta=beta)
