...
step_size = 25
num_epoch = 50
result_dir = 'results_reg_posneg'
resume = None # 'results_resnet18/20_20_img224_batch16_lr0.1_step20_nepoch40_epoch15_checkpoint.pth.tar'
update_begin = 17
num_pred = 10
alpha_pos = 0.0
alpha_neg = 2.0
...
exp_cond = "{0}_{1}_img{2}_batch{3}_lr{4}_step{5}_nepoch{6}_palpha{7}_nalpha{8}_beta{9}_jo".format(missing, overlabeling, img_size, bsize, lr, step_size, num_epoch, alpha_pos, alpha_neg, beta)
...
class my_criterion():
    def __init__(self, alpha_pos=0.0, alpha_neg=0.0, beta=0.0):
    ...
    def __call__(self, preds, targets):
        ...
        prob_entropy = -torch.sum(probs*torch.log(probs)*self.alpha_pos*targets + probs*torch.log(probs)*self.alpha_neg*(1-targets)) / (probs.shape[0]*probs.shape[1])
        prob_avg = torch.mean(probs, dim=0)
        prob_dist = -torch.sum(self.anno_dist * torch.log(prob_avg))
        
        return bce + prob_entropy + self.beta*prob_dist
...
criterion = my_criterion(alpha_pos=alpha_pos, alpha_neg=alpha_neg, beta=beta)
